    ...
    <section class="container project-container mx-auto -mt-56">
      <div class="flex mt-3">
        <div class="w-3/4 mr-6">
          ...
          <div class="flex -mx-2">
            <div 
              v-for="image in campaign.data.images" :key="image.image_url" @click="changeImage($axios.defaults.baseURL + '/' +image.image_url)"
              class="relative w-1/4 bg-white m-2 p-2 border border-gray-400 rounded-20">
              <figure class="item-thumbnail">
                <img :src="$axios.defaults.baseURL + '/' + image.image_url" alt="" class="rounded-20 w-full" />
              </figure>
            </div>
          </div>
        </div>
        <div class="w-1/4">
          <div class="bg-white w-full p-5 border border-gray-400 rounded-20 sticky" style="top: 15px">
            ...
            <div class="flex mt-3">
              <div class="w-1/4">
                <img :src="$axios.defaults.baseURL + '/' + campaign.data.user.image_url" alt="" class="w-full inline-block rounded-full" />
              </div>
            ...
            <ul class="list-check mt-3">
              <li v-for="perk in campaign.data.perks" :key="perk.id">{{ perk }}</li>
            </ul>
            <template v-if="this.$store.state.auth.loggedIn">
              ...
              <a 
                @click="fund"
                class="cursor-pointer text-center mt-3 button-cta block w-full bg-orange-button hover:bg-green-button text-white font-medium px-6 py-3 text-md rounded-full">
                Fund Now
              </a>
            </template>
            <template v-else>
              <nuxt-link 
                to="this.$router.push({ path: '/login' })"
